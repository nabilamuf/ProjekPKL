<?php 
    ...
    class lsp{
        ...
        public function login($username,$password){
            ...
            $_SESSION['kd_user'] = $assoc['kd_user'];
        ...
        public function register($kd_user, $nip, $name,$username,$password,$confirm, $foto, $level,$redirect){
            ...
            if($rows == 0){
                ...
                if($password == $confirm){
                    ...
                    $response = $rg->validateImageus();
                    $response = $rg->validateImage();
                    $response = $rg->validateImagee();
                    $sql = "INSERT INTO table_user VALUES('$kd_user','$nip','$name','$username','$password','$response[image]', '$level')";
                    $query   = mysqli_query($con,$sql);
                    if($query){
        ...
        public function autokodegns($table,$field,$pre){
            global $con;
            $sqlc   = "SELECT COUNT($field) as jumlah FROM $table";
            $querys = mysqli_query($con,$sqlc);
            $number = mysqli_fetch_assoc($querys);
        ...
        public function autokodee($table,$field,$pre){
            global $con;
            $sqlc   = "SELECT COUNT($field) as jumlah FROM $table";
            $querys = mysqli_query($con,$sqlc);
            $number = mysqli_fetch_assoc($querys);
        ...
        public function autokodeus($table,$field,$pre){
            global $con;
            $sqlc   = "SELECT COUNT($field) as jumlah FROM $table";
            $querys = mysqli_query($con,$sqlc);
            $number = mysqli_fetch_assoc($querys);
        ...
        public function selek($table,$user,$kd_user){
            global $con;
            $sql = "SELECT * FROM $table where $user ='$kd_user'";
            $query = mysqli_query($con,$sql);
            $data = [];
            while($bigData = mysqli_fetch_assoc($query)){
                $data[] = $bigData;
        ...
        function validateImagee(){
            global $con;

            $name       = $_FILES['gambar_service']['name']; 
            $ukuranFile = $_FILES['gambar_service']['size']; 
            $error      = $_FILES['gambar_service']['error']; 
            $tmpName    = $_FILES['gambar_service']['tmp_name']; 
        ...
        function validateImages(){
            ...
            $name       = $_FILES['slip_bukti']['name']; 
            $ukuranFile = $_FILES['slip_bukti']['size']; 
            $error      = $_FILES['slip_bukti']['error']; 
            $tmpName    = $_FILES['slip_bukti']['tmp_name']; 
